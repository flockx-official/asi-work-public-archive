...
for (const {
  ...
  test(name, { skip: process.platform !== 'darwin' }, async (t) => {
    ...
    recordBundle(directory, 'ASI-Work.zip', appMinimum, backendMinimum);
    recordBundle(directory, 'ASI-Work_intel_mac.zip', intelMinimum, backendMinimum);
...
test('does not publish a manifest if one architecture has no compatibility metadata', (t) => {
  const directory = workspace(t);
  fs.writeFileSync(
    path.join(directory, 'ASI-Work.zip.macos.json'),
...
test(
  ...
  (t) => {
    for (const minimum of ['13.1', '13.0.1', '16.0']) {
      const directory = workspace(t);
      assert.throws(() => recordBundle(directory, 'ASI-Work.zip', minimum, '12.0'));
      assert.equal(fs.existsSync(path.join(directory, 'ASI-Work.zip.macos.json')), false);
    }
  }
);
